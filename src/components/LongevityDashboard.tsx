  ...
  const insights = useMemo(() => {
    ...
    if (hba1c) {
      ...
      if (v > 5.7) {
        ...
        riskFactors.push("Glicada acima do ideal");
      } else if (v < 5.4) {
        bioAgeModifier -= 1;
        protectiveFactors.push("Glicemia Otimizada");
      }
    }
    ...
    if (crp) {
      ...
      if (v > 3) {
        ...
        riskFactors.push("Inflamação Sistêmica Alta (PCR)");
      } else if (v < 1) {
        bioAgeModifier -= 1;
        protectiveFactors.push("Baixa Inflamação Sistêmica");
      }
    }
    ...
    if (hdl) {
      ...
      if (v < 40) {
        bioAgeModifier += 2;
        cardiovascularRisk += 10;
        riskFactors.push("HDL baixo");
      } else if (v >= 60) {
        bioAgeModifier -= 2;
        cardiovascularRisk -= 5;
        protectiveFactors.push("HDL Cardioprotetor");
      }
    }
    ...
    if (vitd) {
      ...
      if (v < 30) {
        bioAgeModifier += 1;
        riskFactors.push("Vitamina D Baixa");
      } else if (v >= 40) {
        bioAgeModifier -= 1;
        protectiveFactors.push("Vitamina D Excelente");
      }
    }
  ...
  const chartData = useMemo(() => {
    ...
    sorted.forEach((exam) => {
      ...
      const val = parseFloat(
        exam.resultado.replace(/[^0-9,.-]/g, "").replace(",", "."),
      );
      if (isNaN(val)) return;

      const name = exam.nomeExame.toLowerCase();
      if (name.includes("glicada") || name === "a1c")
        grouped[exam.dataExame].HbA1c = val;
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8 pb-20">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-100 tracking-tight">
            Dashboard de Longevidade
          </h1>
          <p className="text-sm font-medium text-slate-400">
            Estimativas baseadas nos seus exames laboratoriais mais recentes
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <motion.div
          ...
          className="bg-slate-800/40 backdrop-blur-md rounded-3xl p-6 shadow-lg shadow-black/20 border border-white/10 flex flex-col justify-between overflow-hidden relative"
        >
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="flex items-center gap-2 text-sm font-bold text-slate-300 uppercase tracking-wider">
            ...
            <p className="mt-4 text-sm text-slate-400 font-medium">
              Sua idade cronológica base é{" "}
              <span className="font-bold text-slate-200">
                ~{insights.chronologicalAge} anos
              </span>
              .
        ...
        <motion.div
          ...
          className="bg-slate-800/40 backdrop-blur-md rounded-3xl p-6 shadow-lg shadow-black/20 border border-white/10 flex flex-col justify-between overflow-hidden relative"
        >
          <div className="absolute -right-10 -top-10 w-40 h-40 bg-rose-900/30 rounded-full blur-3xl opacity-60 pointer-events-none"></div>
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="flex items-center gap-2 text-sm font-bold text-slate-300 uppercase tracking-wider">
            ...
            <div className="mt-6 flex items-baseline gap-2">
              <span
                className={`text-6xl font-black tracking-tighter ${insights.cardiovascularRisk > 20 ? "text-rose-600" : insights.cardiovascularRisk > 10 ? "text-amber-500" : "text-emerald-500"}`}
              >
                {insights.cardiovascularRisk}%
              </span>
            </div>

            <div className="mt-6 h-3 w-full bg-slate-700/60 rounded-full overflow-hidden flex">
              <div
              ...
              {/* Pointer */}
              <div
                className="absolute w-1 h-5 bg-slate-800 rounded-full transform -translate-y-1 shadow-2xl shadow-black/60 shadow-black/40 transition-all duration-1000"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <motion.div
          ...
          className="bg-emerald-900/30/50 rounded-3xl p-6 border border-emerald-100"
        >
          ...
          {insights.protectiveFactors.length > 0 ? (
            <ul className="space-y-3">
              {insights.protectiveFactors.map((f, i) => (
                <li
                  key={i}
                  className="flex items-center gap-3 text-sm font-semibold text-emerald-300 bg-slate-800/40 backdrop-blur-md p-3 rounded-xl shadow-lg shadow-black/20 border border-white/10"
                >
            ...
            <p className="text-sm text-emerald-600/70 font-medium">
              Nenhum fator protetor forte detectado nos exames recentes.
            </p>
          )}
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-rose-900/30/50 rounded-3xl p-6 border border-rose-100"
        >
          ...
          {insights.riskFactors.length > 0 ? (
            <ul className="space-y-3">
              {insights.riskFactors.map((f, i) => (
                <li
                  key={i}
                  className="flex items-center gap-3 text-sm font-semibold text-rose-300 bg-slate-800/40 backdrop-blur-md p-3 rounded-xl shadow-lg shadow-black/20 border border-white/10"
                >
            ...
            <p className="text-sm text-rose-600/70 font-medium">
              Excelente! Nenhum fator de risco severo detectado.
            </p>
          )}
        </motion.div>
      </div>
      ...
      {chartData.length > 1 && (
        <motion.div
          ...
          className="bg-slate-800/40 backdrop-blur-md rounded-3xl p-6 shadow-lg shadow-black/20 border border-white/10"
        >
          <div className="mb-6">
            ...
            <p className="text-sm text-slate-400 font-medium">
              Evolução de marcadores chave de longevidade ao longo do tempo
            </p>
          </div>
          ...
          <div className="flex gap-4 justify-center mt-4 text-xs font-bold text-slate-400">
            {chartData.some((d) => d.HbA1c) && (
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-md bg-violet-500"></div> HbA1c
                (Glicada)
              </div>
            )}
            {chartData.some((d) => d.VitD) && (
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-md bg-amber-500"></div> Vitamina
                D
              </div>
            )}
          </div>
